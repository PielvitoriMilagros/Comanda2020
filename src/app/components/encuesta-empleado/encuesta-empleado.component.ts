...
export class EncuestaEmpleadoComponent implements OnInit {
  ...
  public mensaje;
  public areas=['banio','cocina','salon','descanso'];
  ...
  ngOnInit() {
    this.imagen = '../../../assets/imagenes/encEmpleado.png';
  ...
  validarAlta(){
    ...
    if(this.camposValidados()){
      ...
      this.firestore.saveEncuestas(encuesta.toJson()).then(resp=>{
        Swal.fire({
          title: 'Felicidades',
          text: 'Las respuestas fueron guardada correctamente',
          icon: 'success'
        }).then(()=>{
  ...
  camposValidados(){
    ...
    if(this.imagen == "../../../assets/imagenes/encEmpleado.png"){
